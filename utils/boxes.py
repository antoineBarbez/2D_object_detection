...
def clip_to_window(boxes, window):
    ...
    window_max_boudaries = tf.stack([win_x_max, win_y_max, win_x_max, win_y_max])
    window_min_boudaries = tf.stack([win_x_min, win_y_min, win_x_min, win_y_min])
...
def decode(boxes, reference_boxes):
    ...
    return tf.concat([centers - 0.5 * sizes, centers + 0.5 * sizes], axis=-1)
